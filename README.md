...
Rebuilds are triggered only if Ubuntu builds a new image, or if one of the [`ubuntu-oci` base packages](https://git.launchpad.net/cloud-images/+oci/ubuntu-base/) were updated. Currently we create images for **Ubuntu 26.04 "Resolute Raccoon"**, **Ubuntu 24.04 "Noble Numbat"** and **Ubuntu 22.04 "Jammy Jellyfish"**. Please note that we might add or drop branches at any time, but usually around upstream's release resp. end-of-life dates.
